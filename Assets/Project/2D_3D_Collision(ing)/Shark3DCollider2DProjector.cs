...
public class Shark3DCollider2DProjector : MonoBehaviour
{
    ...
    [SerializeField] private Camera    mainCamera        = null; // 메인 카메라 (원근 투영)
    [SerializeField] private Transform player2DTransform = null; // 2D 플레이어 Transform

    [Header("Collider Settings")]
    [SerializeField] private bool autoFindColliders = true; // 시작 시 자동으로 자식 콜라이더 찾기

    [Header("Debug Options")]
    [SerializeField] private bool  debugMode       = true;           // 디버그 모드
    [SerializeField] private Color debugLineColor  = Color.yellow;   // 디버그 라인 색상
    [SerializeField] private Color debugPointColor = Color.green;    // 디버그 포인트 색상
    [SerializeField] private float debugPointSize  = 0.3f;           // 디버그 포인트 크기
    ...
    void ProjectAllCollidersTo2DPlane()
    {
        foreach (Collider2DData data in collider2DList)
        {
            ...
            // 디버그: X축 90도 콜라이더만 로그 출력
            if (debugMode && data.name.Contains("x rotation 90"))
            {
                Debug.Log($"[DEBUG] {data.name}");
                Debug.Log($"  offsetFromShark: {data.offsetFromShark}");
                Debug.Log($"  rotatedOffset: {rotatedOffset}");
                Debug.Log($"  worldPos3D: {worldPos3D}");
                Debug.Log($"  projectedWorldPos: {projectedWorldPos}");
                Debug.Log($"  originalLocalScale: {data.originalLocalScale}");
            }
            ...
            if (debugMode && data.name.Contains("x rotation 90"))
            {
                ...
                Debug.Log($"  newScale: {data.originalLocalScale * scaleRatio}");
            }
    ...
    void DrawColliderGrid(Collider2DData data)
    {
        ...
        bool isRotated90 = data.name.Contains("x rotation 90") ||
                          data.name.Contains("+ x rotation 90") ||
                          data.name.Contains("rotation 90");
    ...
    /// 채워진 박스 그리기 (회전 적용)
    /// </summary>
    void DrawFilledBox(Vector3 center, float width, float height, Quaternion rotation, Color fillColor, Color lineColor)
    {
        // 박스의 4개 꼭지점 (로컬 좌표)
    ...
    {
        FindAllColliders2D();
        Debug.Log($"[Shark3DProjector] 콜라이더 리스트 갱신 완료: {collider2DList.Count}개");
    }
    ...
    {
        Debug.Log("═══════════════════════════════════════════════");
        Debug.Log($"[Shark3D Projector Debug Info]");
        Debug.Log($"Player 2D Depth: {player2DDepth}");
        Debug.Log($"Collider Count: {collider2DList.Count}");

        foreach (Collider2DData data in collider2DList)
        {
            if (data.transform == null) continue;

            float scaleRatio = CalculateScaleRatio(data.transform.position);
            Debug.Log($"  - {data.name}");
            Debug.Log($"    Position: {data.transform.position}");
            Debug.Log($"    Scale Ratio: {scaleRatio:F3}");
            Debug.Log($"    Current Scale: {data.transform.localScale}");
        }
        Debug.Log("═══════════════════════════════════════════════");
    }
}